        ...
        if beliefs:
            ...
            for player_id in sorted(beliefs.keys()):
                ...
                for target_id, wolf_prob in enumerate(p_wolf):
                    if target_id != player_id:  # 不显示对自己的belief
...
def broadcast_death(victim_id, death_cause="unknown"):
    ...
    if victim_id >= 0 and victim_id < game_state.num_players:
        ...
        if game_state.game_mode == 'human':
            ...
            # === 修改：区分人类玩家和AI玩家的遗言处理 ===
            # 只有夜晚被杀的第一个死者才能留遗言
            if (death_cause == "night_kill" and not game_state.legacy_spoken):
                ...
                if is_first_night_death:
                    ...
                    if (game_state.human_handler and 
                        victim_id == game_state.human_handler.human_player_id):
                        # === 人类玩家死亡：手动选择遗言 ===
                        
                        # 如果死者是预言家，先显示查验结果
                        human_role = game_state.players[victim_id]["role"]
                        if human_role == Role.SEER:
                            # 显示预言家查验结果
                            if hasattr(game_state, 'night_actions') and str(victim_id) in game_state.night_actions:
                                target_id = game_state.night_actions[str(victim_id)]
                                if target_id < game_state.env.N:
                                    ...
                                    broadcast_host_message(
                                        f"【预言家查验结果】{victim['name']} 昨晚查验了{target_name}，结果是{result_text}",
                                        "seer_result"
                                    )
...
def on_start_game(data):
    ...
    try:
        ...
        for pid, p in game_state.players.items():
            ...
            if game_mode == 'human':
                # 人机模式：根据人类玩家身份决定可见性
                human_role = game_state.players[human_player_id]["role"]
                
                if pid == human_player_id:
                    # 人类玩家自己的身份总是可见
                    visible_role = p["role"].name
                elif human_role == Role.WOLF and p["role"] == Role.WOLF:
                    # 如果人类是狼人，可以看见所有狼人队友
                    visible_role = p["role"].name
                else:
                    # 其他情况身份不可见
                            ...
                            if isinstance(event, dict):
                                if event.get("phase") == "seer_check":
                                    ...
                                    if victim_id is not None and victim_id >= 0:
                                        ...
                                        # 移除显示角色信息的代码
                                        broadcast_host_message(f"昨晚，{victim_name} 被狼人杀害", "death_announce")
                        ...
                        if first_night_victims:
                            ...
                            for victim_id in first_night_victims:
                                if (game_state.game_mode == 'human' and 
                                    game_state.human_handler and 
                                    victim_id == game_state.human_handler.human_player_id):
                                    human_victims.append(victim_id)
                                else:
                                    ai_victims.append(victim_id)
                            
                            # 处理AI死者的自动遗言
                            if ai_victims:
                                broadcast_host_message("第一夜的死者留下遗言：", "legacy")
                                
                                for victim_id in ai_victims:
                            ...
                            # 人类死者的遗言已经在 broadcast_death 中处理了，这里不需要额外处理
