...
class HumanAIGameHandler:
    ...
    def handle_last_words_human(self):
        ...
        # 重构遗言选项数据结构
        legacy_options = []
        human_role = self.game_state.players[self.human_player_id]["role"]
        
        # 预言家遗言特殊选项
        if human_role == Role.SEER and hasattr(self.game_state.env, 'seer_records'):
            # 添加验人记录选项
            for record in self.game_state.env.seer_records:
                if record[0] == self.human_player_id:  # [seer_id, target_id, result, day]
                    target_id = record[1]
                    legacy_options.append({
                        'type': 'reveal_check',
                        'day': record[3],
                        'target': target_id,
                        'name': f"公布第{record[3]}天对{self.game_state.players[target_id]['name']}的查验结果"
                    })

        # 添加指控选项（仅限存活玩家）
        alive_targets = [
            {'type': 'accuse', 'target': i, 'name': f"指控{self.game_state.players[i]['name']}"}
            for i in range(self.game_state.num_players)
            if self.game_state.env.alive[i] and i != self.human_player_id
        ]
        
        # 合并选项
        legacy_options.extend(alive_targets)
        legacy_options.append({'type': 'pass', 'name': '保持沉默'})

        # 发送结构化遗言选项
        self.broadcast_human_action_request('last_words', {
            'options': legacy_options,
            'max_select': 1  # 只能选择一个遗言内容
        })
        return True
        ...
        try:
            if action_type == 'wolf_kill':
                ...
                if target_id < self.game_state.env.N:
                    ...
                    is_wolf = (target_role == Role.WOLF)
                    result_text = "狼人" if is_wolf else "好人"
                    
                    # 显示查验选择
                    emit('host_message', {
                        'message': f"你选择查验 {target_name}",
                        'phase': 'seer_turn'
                    }, broadcast=True)
                    
                    # 立即显示查验结果
                    emit('host_message', {
                        'message': f"【查验结果】{target_name} 是 {result_text}",
                        'phase': 'seer_result'
                    }, broadcast=True)
            ...
            if target_id is not None and target_id >= 0 and check_result is not None:
                ...
                if human_player["role"] == Role.SEER:
                    self.game_state.env.seer_records.append((
                        ...
                        target_id,                # 查验目标
                        role_int ,                 # 查验结果
                        self.game_state.day - 1   # 查验发生的夜晚（上一天）
                    ))
    ...
    def _process_human_legacy(self, data):
        """处理人类玩家的遗言"""
        ...
        if option_type == 'reveal_check' and day is not None:
            # 处理公布查验结果
            record = next((r for r in self.game_state.env.seer_records 
                          if r[0] == self.human_player_id and r[3] == day), None)
            if record:
                target_name = self.game_state.players[record[1]]['name']
                result = "狼人" if record[2] == Role.WOLF else "好人"
                message = f"{prefix}{speaker_name} 公布：第{day}天查验{target_name}是{result}！"
                
                # 记录到public_log
                log_entry = (0, self.human_player_id, int(TalkType.REVEAL_CHECK), record[1], record[2])
                self.game_state.env.public_log.append(log_entry)
                
        elif option_type == 'accuse' and target_id is not None:
            # 处理指控
            target_name = self.game_state.players[target_id]['name']
            ...
            emit('host_message', {'message': message, 'phase': 'legacy'}, broadcast=True)
            self._update_beliefs_from_events([log_entry])  # 更新信念
