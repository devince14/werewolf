...
from competition_template.agents.belief_agent import BeliefState
...
class User0Agent(BaseAgent):
    ...
    def act(self, env):
        ...
        if env.stage == "talk":
            ...
            # 如果不是当前发言者，返回无效动作
            if User0Agent.current_speaker != self.agent_id:
                return np.array([0, TalkType.CLAIM_GOOD, env.N])
    ...
    def _record_legacy(self, env, talk_type: int, target: int):
        ...
        for agent in env.agents:
            ...
            if hasattr(agent, 'strategies') and agent.strategies.get('belief_update'):
                # 添加类型检查
                if not isinstance(agent, User0Agent):  # 新增检查
